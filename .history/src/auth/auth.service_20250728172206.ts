...
export class AuthService {
  ...
  private async checkOtpRateLimit(email: string): Promise<void> {
    ...
    const dailyLimitKey = `daily_limit:${email}`;

    // Check recent requests
    const recentRequest = await this.redisService.exists(rateLimitKey);
    if (recentRequest) {
      const remainingTime = await this.redisService.getTTL(rateLimitKey);
      throw new BadRequestException(
    ...
    // Check daily limit
    const dailyCount = (await this.redisService.get(dailyLimitKey)) || 0;
    if (dailyCount >= 5) {
      throw new BadRequestException('Daily OTP limit exceeded');
    }
  ...
  // Debug method - see what's in Redis
  async getRedisDebugInfo(): Promise<any> {
    const keys = await this.redisService.keys('*');
    const data = {};

    for (const key of keys) {
      data[key] = await this.redisService.get(key);
    }

    return { totalKeys: keys.length, data };
  }
}
